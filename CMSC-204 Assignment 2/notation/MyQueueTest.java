...
public class MyQueueTest {
	...
	public Double kobe = 24.0, kyrie = 11.0, juicewrld = 999.0, durant = 35.0, 
			real = 100.0, westbrook = 0.0;
	...
	public void setUp() throws Exception {
	    ...
	    doubleQ.enqueue(kobe);
	    doubleQ.enqueue(kyrie);
	    doubleQ.enqueue(juicewrld);
	}
	...
	public void testDequeueStudent() {
		...
		try {
		      assertEquals(kobe, doubleQ.dequeue());
		      assertEquals(kyrie, doubleQ.dequeue());
		      assertEquals(juicewrld, doubleQ.dequeue());
		      // Queue is empty, next statement should cause QueueUnderFlowException
		      doubleQ.dequeue();
		      assertTrue("This should have caused an QueueUnderflowException", false);
		    } catch (QueueUnderflowException e) {
	...
	public void testEnqueueStudent() {
		...
		try {
			...
			assertEquals(true, doubleQ.enqueue(durant));
			assertEquals(4, doubleQ.size());
			assertEquals(true, doubleQ.enqueue(real));
			assertEquals(5, doubleQ.size());
			// Queue is full, next statement should cause QueueOverFlowException
			doubleQ.enqueue(westbrook);
			assertTrue("This should have caused an QueueOverflowException", false);
		} catch (QueueOverflowException e) {
			assertTrue("This should have caused an QueueOverflowException", true);
		} catch (Exception e) {
	...
	public void testToStringStudent() throws QueueOverflowException {
		//Use the doubleQ for student tests
		assertEquals("24.011.0999.0", doubleQ.toString());
	    doubleQ.enqueue(durant);
	    assertEquals("24.011.0999.035.0", doubleQ.toString());
	    doubleQ.enqueue(real);
	    assertEquals("24.011.0999.035.0100.0", doubleQ.toString());
	}
